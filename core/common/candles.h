...
constexpr std::chrono::minutes tf_duration(Timeframe tf)
{
    ...
    switch (tf)
    {
        case Timeframe::M1: return 1min;
        case Timeframe::M5: return 5min;
        case Timeframe::M15: return 15min;
        case Timeframe::H1: return 60min;
        case Timeframe::D1: return 1440min;
    }
...
struct Candle
{
    ...
    Candle() = default;
    Candle(Timestamp timestamp_utc, double o, double h, double l, double c, double v, Timeframe timeframe)
    ...
    [[nodiscard]] constexpr double upperWick() const noexcept { return m_high - std::max(m_open, m_close); }

    [[nodiscard]] constexpr double lowerWick() const noexcept { return std::min(m_open, m_close) - m_low; }

    [[nodiscard]] constexpr double mid() const noexcept { return (m_open + m_close) * 0.5; }
    ...
    [[nodiscard]] constexpr bool is_doji(double eps = 1e-9) const noexcept { return body() <= eps; }
